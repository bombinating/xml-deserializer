...
The library uses a simple DSL to map element names to handler lambdas, using a context object that the lambdas populate, without exposing the low-level XML processing details.
...
The goal of the XD library is to provide a high-level API that does not require knowledge of XML processing. The XML processing is hidden by the library and the use of the library is wholly programmatic, without any annotations or XML configuration. In addition, the processing does not require an XSD and can proceed piecemeal, only mapping the elements that are of interest, rather than mapping everything.
